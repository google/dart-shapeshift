...
class ClassesReporter {
  ...
  void reportEachClass(String klassName, DiffNode klass) {
    ...
    if (klass.hasChanged) {
      klass.changed.forEach((String key, List oldNew) {
        ...
        if (qualifiedName == null && klass.changed.containsKey('qualifiedName'))
  ...
  void reportAddedClassMembers(String qualifiedName, DiffNode klass) {
    ...
    klass.added.forEach((String key, dynamic thing) {
      ...
      String classThingLink =
          mdLinkToDartlang(qualifiedName, name);
      io.writeln(
  ...
  void reportChangedTypedefParameter(String qualifiedName, String name, DiffNode parameter) {
    ...
    io.writeln(
        'The [$qualifiedName](#) $category\'s [$name](#) '
